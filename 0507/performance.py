...
# 定义文件路径
time_error_log_file_path_own = r".\自有方法\timeError.log"
time_error_log_file_path_chrony = r".\chrony\timeError.log"
ptp4l_log_file_path = r".\自有方法\ptp4l.log"
...
def process_ptp4l_log(file_path, time_range=None):
    if os.path.exists(file_path):
        ...
        for line in filtered_lines:
            ...
            # 转化为浮点数并从ns转为μs
            time = float(time)
            master_offset = float(master_offset) / 1000  # ns转μs
            frequency = float(frequency) / 1000          # ns转μs
            delay = float(delay) / 1000                  # ns转μs
            data.append([time, master_offset, frequency, delay])

        # 创建 DataFrame
        df = pd.DataFrame(data, columns=["Time", "Master Offset (μs)", "Frequency (μs)", "Delay (μs)"])
        # 调整时间轴：减去第一个时间值
        ...
        # 绘制 master offset 的图
        plt.plot(df["Time"], df["Master Offset (μs)"], label="PTP Master Offset (μs)", color='red')

        # 输出统计特性
        mean_offset = df["Master Offset (μs)"].mean()
        std_offset = df["Master Offset (μs)"].std()
        print(f"PTP - Mean Master Offset (μs): {mean_offset}")
        print(f"PTP - Standard Deviation of Master Offset (μs): {std_offset}")
    else:
        print(f"文件不存在: {file_path}")
...
# 处理 ptp4l.log
process_ptp4l_log(ptp4l_log_file_path)  # 可选的时间范围
...
plt.grid()
#导出矢量图
# plt.savefig(r".\comparison_all_metrics2.svg")
# plt.savefig(r".\comparison_all_metrics_adjusted2.png")
plt.show()